      ...
      <div className="flex flex-col md:flex-row gap-12 items-center">
        <div className="w-full md:w-1/2 flex flex-col gap-6">
          <div className="inline-flex w-fit items-center gap-2 rounded-full border border-purple-500/30 bg-purple-50 px-3 py-1 text-xs font-medium text-purple-600">
            ...
            <span>{t('home.focusModeBadge')}</span>
          </div>
          <h2 className="text-3xl font-black text-gray-900 tracking-tight">{t('home.focusModeTitle')}</h2>
          <p className="text-lg text-gray-600 leading-relaxed">
            {t('home.focusModeSubtitle')}
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
            <div className="p-4 rounded-lg bg-gray-50 border border-gray-200">
              <span className="material-symbols-outlined text-primary mb-2">timer</span>
              <h4 className="font-bold text-gray-900">{t('home.focusPomodoro')}</h4>
              <p className="text-xs text-gray-500 mt-1">{t('home.focusPomodoroDesc')}</p>
            </div>
            <div className="p-4 rounded-lg bg-gray-50 border border-gray-200">
              <span className="material-symbols-outlined text-primary mb-2">block</span>
              <h4 className="font-bold text-gray-900">{t('home.focusAppBlock')}</h4>
              <p className="text-xs text-gray-500 mt-1">{t('home.focusAppBlockDesc')}</p>
            </div>
          </div>
        </div>
